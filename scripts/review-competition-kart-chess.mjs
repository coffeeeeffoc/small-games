...
try {
  const [a, b, outsider] = await Promise.all([0, 1, 2].map(() => request('/sessions/guest', null, {})));
  assert.notEqual(a.playerId, b.playerId);
  await request('/me', '0'.repeat(64), undefined, 401);
  await request('/sessions/platform', null, { platform: 'wechat', appId: 'review-unconfigured', code: 'test' }, 503);
  for (const path of ['/internal/verify', '/internal/kart-results']) await request(path, null, {}, 404);
  ...
  assert.equal((await request(`/rooms/${room.code}/ready`, b.token, {})).startedAt, playing.startedAt);
  const move = (player, seq, index, status = 200) => request(`/rooms/${room.code}/actions`, player.token, { seq, action: { type: 'deploy-directly', to: index } }, status);
  await request(`/rooms/${room.code}/actions`, a.token, { seq: 1, action: { type: 'score', score: 999999 } }, 422);
  await request(`/rooms/${room.code}/actions`, outsider.token, { seq: 1, action: { type: 'deploy-directly', to: 0 } }, 403);
  ...
  record('wrong game, duplicate join, full room, outsider, wrong turn, illegal location, seq, duplicate ready, abandonment');
  ...
  try {
    ...
    await page.waitForFunction(() => document.querySelector('[data-status]').textContent.includes('邀请链接已复制'));
    ...
    const inviteeContext = await browser.newContext({ viewport: { width: 390, height: 844 }, hasTouch: true });
    ...
    assert.notEqual(await invitee.evaluate(async () => (await __competition.session()).playerId), identity);
    assert.equal((await page.evaluate(code => __competition.request(`/rooms/${code}`), code)).players.length, 2);
    record('real copied H5 invitation opens in a second independent browser identity and joins its room', { code });
    ...
    assert.equal((await page.evaluate(code => __competition.request(`/rooms/${code}`), code)).status, 'waiting');
    await page.reload();
    await page.locator('[data-competition-launch]').tap();
    assert.equal(await page.evaluate(async () => (await __competition.session()).playerId), identity);
    await page.locator('[data-room-code]').waitFor({ state: 'visible' });
    assert((await page.locator('[data-room-code]').innerText()).includes(code));
    assert.equal((await page.evaluate(code => __competition.request(`/rooms/${code}`), code)).players.length, 2);
    assert.deepEqual(errors, []);
    record('real Chrome Fullscreen API entry and system exit, persistent identity and room rejoin on reload', { code, identity, browser: browser.version() });
    const wrapper = createServer((_request, response) => {
      response.writeHead(200, { 'content-type': 'text/html', 'permissions-policy': 'fullscreen=()' });
      response.end(`<iframe src="${origin}/games/xiangqi-five/" style="width:390px;height:844px;border:0"></iframe>`);
    });
    ...
    try {
      ...
      assert.equal(await frame.locator('body').evaluate(() => Boolean(document.fullscreenElement)), false);
      await frame.locator('[data-ready]').tap();
      await frame.locator('[data-ready]:disabled').waitFor();
      await denied.screenshot({ path: fileURLToPath(new URL('xiangqi-fullscreen-denied.png', directory)) });
      record('real Permissions-Policy denial shows accurate feedback and preserves focus controls', { feedback });
    } finally { await new Promise(resolve => wrapper.close(resolve)); }
