...
const html = (content) =>
  `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><body>${content}<script src="/fullscreen.js"></script></body></html>`;
...
const server = createServer((request, response) => {
  response.setHeader(
    'Content-Type',
    request.url === '/fullscreen.js' ? 'text/javascript' : 'text/html; charset=utf-8',
  );
  response.end(
    request.url === '/fullscreen.js'
      ? script
      : html(
          request.url === '/host'
...
try {
  ...
  for (const embedded of [false, true]) {
    ...
    results.push({
      entry: embedded ? 'same-origin iframe host' : 'standalone',
  ...
  for (const mode of ['unsupported', 'rejected', 'no-event']) {
    ...
    await expect(page.locator('#game-display-notice')).toContainText(
      mode === 'unsupported' ? '不支持' : mode === 'rejected' ? '未允许' : '未切换',
    );
    ...
    results.push({
      branch: mode,
      simulatedCapability: true,
      playable: true,
      noFalseFullscreen: true,
    });
    await page.close();
  }
  ...
  await writeFile(
    new URL('fullscreen-controller.json', output),
    JSON.stringify({ date: new Date().toISOString(), physicalMobile: false, results }, null, 2),
  );
