...
async function makePlayer(game, suffix, width = 390, height = 844) {
  ...
  page.on('response', async (response) => {
    if (response.url().includes('/api/competition/v1') && !response.ok())
      report.errors.push({
        game,
        player: suffix,
  ...
  if (
    game === 'cops-robbers' &&
    (await page.locator('body').evaluate((element) => element.classList.contains('focus-play')))
  )
...
async function snap(player, name) {
  await player.page.screenshot({
    path: new URL(`${player.game}-${name}-${player.suffix}.png`, directory).pathname.replace(
      /^\/([A-Za-z]:)/,
      '$1',
    ),
  });
  await writeFile(
    new URL(`${player.game}-${name}-${player.suffix}.json`, directory),
    JSON.stringify(
      { room: player.room, text: await player.page.locator('dialog[open]').innerText() },
      null,
      2,
    ),
  );
...
async function start(game) {
  const a = await makePlayer(game, 'a'),
    b = await makePlayer(game, 'b');
  await a.page.locator('[data-create]').click();
  try {
    await expect.poll(() => a.room?.code, { timeout: 15000 }).toBeTruthy();
  } catch (error) {
    await snap(a, 'create-failed');
    throw error;
  }
  ...
  record(`${game}: two browser identities created, joined and ready`, {
    code: a.room.code,
    ids: [a.room.players[a.room.you].id, b.room.players[b.room.you].id],
  });
  await snap(a, 'playing');
  await snap(b, 'playing');
  return [a, b];
}
async function clickCanvas(player, x, y, action = null) {
  ...
  assert(
    ...
    `outside visible canvas: ${x},${y} in ${rect.width}x${rect.height}`,
  );
    ...
    try {
      await expect
        .poll(
          () =>
            player.room.seq > seq ||
            player.room.state.finished ||
            player.room.status === 'finished',
          { timeout: 15000 },
        )
        .toBeTruthy();
    } catch (error) {
      await snap(player, 'action-failed');
      console.error({ action, x, y, rect, seq, completed: player.actions.length });
      throw error;
    }
...
async function playCops(player) {
  ...
  for (const plan of solution) {
    ...
    const cop = Math.max(
      0,
      plan.findIndex((target, index) => target !== board.cops[index]),
    );
    const rect = await player.page.locator('[data-play]').boundingBox();
    const size = Math.max(100, Math.min(rect.width - 16, rect.height - 122, 620));
    const point = (index) => ({
      x: (rect.width - size) / 2 + (level.nodes[index].x / 600) * size,
      y: 45 + (level.nodes[index].y / 600) * size,
    });
    const from = point(board.cops[cop]),
      target = point(plan[cop]);
  ...
  record(`cops-robbers/${player.suffix}: canvas capture completed`, {
    steps: player.actions.length,
    result: player.room.players[player.room.you].result,
  });
}
const translations = {
  森林: 'forest',
  ...
  wordLoop: while (!player.room.state.finished) {
    ...
    for (const char of answer) {
      ...
      const tile = state.tiles.find(
        (tile) => tile.char === char && !tile.blocked && !state.selected.includes(tile.id),
      );
      assert(tile, `uncovered ${char} for ${answer}`);
      const rect = await player.page.locator('[data-play]').boundingBox();
      const scale = Math.min(
        (rect.width - 20) / state.board.width,
        (rect.height - 226) / state.board.height,
      );
      await clickCanvas(
        player,
        (rect.width - state.board.width * scale) / 2 + (tile.x + tile.size / 2) * scale,
        95 + (tile.y + tile.size / 2) * scale,
        { type: 'select', tileId: tile.id },
      );
    }
    if (player.room.state.finished) break;
    const state = player.room.state,
      rect = await player.page.locator('[data-play]').boundingBox();
    const scale = Math.min(
      (rect.width - 20) / state.board.width,
      (rect.height - 226) / state.board.height,
    );
    await clickCanvas(
      player,
      12 + (rect.width - 36) / 8,
      95 + state.board.height * scale + 20 + 28 + 22,
      { type: 'submit' },
    );
  }
  assert(player.room.state.finished);
  record(
    `letters-words2/${player.suffix}: canvas ${player.room.state.cleanCorrect}/18 ${player.room.state.cleanCorrect === 18 ? 'completion' : 'real timeout result'}`,
    { actions: player.actions.length, result: player.room.players[player.room.you].result },
  );
}
...
async function failures() {
  const a = await makePlayer('cops-robbers', 'crossgame-a'),
    b = await makePlayer('letters-words2', 'crossgame-b');
  await a.page.locator('[data-create]').click();
  ...
  await a.page.waitForResponse(
    (response) =>
      response.url().endsWith(`/rooms/${a.room.code}`) && response.request().method() === 'GET',
  );
  assert.equal(a.room.players.length, 1);
  await snap(a, 'crossgame-fixed');
  await snap(b, 'crossgame-fixed');
  record('FIXED: wrong-game join rejected without occupying second seat', {
    code: a.room.code,
    players: a.room.players.length,
  });
  ...
  await a.page.screenshot({
    path: new URL('cops-robbers-offline-exit-fixed.png', directory).pathname.replace(
      /^\/([A-Za-z]:)/,
      '$1',
    ),
  });
  record(
    'FIXED: offline exit closes modal and leaves accurate feedback',
    await a.page.locator('[data-competition-launch]').innerText(),
  );
  ...
  await expect(a.page.locator('dialog[open]')).toHaveCount(0);
  record(
    'cops-robbers: reconnect resumes original room and confirmed exit is available',
    a.room.code,
  );
...
async function layouts() {
  ...
  const hudX = rect.width * 0.52 + 10,
    hudWidth = rect.width - rect.width * 0.52 - 22;
  ...
  await clickCanvas(a, 12 + (rect.width - 32) / 2 + 8 + (rect.width - 32) / 4, 72, {
    type: 'choose',
    wordId: lastWord,
  });
  ...
  const state = a.room.state,
    tile = state.tiles.find((tile) => !tile.blocked),
    boardWidth = rect.width * 0.52;
  const scale = Math.min(
    (boardWidth - 20) / state.board.width,
    (rect.height - 24) / state.board.height,
  );
  await clickCanvas(
    a,
    (boardWidth - state.board.width * scale) / 2 + (tile.x + tile.size / 2) * scale,
    12 + (tile.y + tile.size / 2) * scale,
    { type: 'select', tileId: tile.id },
  );
  assert.equal(a.room.state.selected.length, 1);
  record('FIXED: landscape paging reaches sixth meaning, returns and accepts a tile', {
    canvas: rect,
    selectedMeaning: a.room.state.meaning,
  });
  ...
  for (const game of ['cops-robbers', 'letters-words2']) {
    ...
    if (game === 'letters-words2') {
      ...
      await page
        .locator('#word-input')
        .fill(
          `apple 苹果，表示一种可以食用的水果，也用于说明这是一条需要折叠展示而不应把操作棋盘推出屏幕的完整长释义。${'补充释义，帮助理解词语在句子中的含义。'.repeat(8)}\nforest 森林，${'指大面积的树木和自然环境，供学习者阅读完整含义。'.repeat(8)}`,
        );
      ...
      for (const [width, height] of [
        ...
        const areas = await page
          .locator('#board,.spelling-area,.board-tools')
        ...
        await page.screenshot({
          path: new URL(`letters-long-meaning-${width}.png`, directory).pathname.replace(
            /^\/([A-Za-z]:)/,
            '$1',
          ),
        });
        for (const area of areas)
          assert(
            area.rect.y >= 0 && area.rect.bottom <= height,
            `${area.class} below ${height}: ${area.rect.bottom}`,
          );
        const covered = await page
          .locator('.board-tools button:not([disabled])')
          .evaluateAll((elements) =>
            elements
              .filter((element) => {
        ...
        const tiles = await page
          .locator('#board button[aria-disabled="false"]')
          .evaluateAll((elements) =>
            elements.map((element) => element.getBoundingClientRect().toJSON()),
          );
        assert(
          tiles.length && tiles.every((tile) => tile.width >= 44 && tile.height >= 44),
          'available letter targets must remain at least 44px',
        );
        ...
        record(
          `letters-words2: long definition keeps core controls visible at ${width}x${height}`,
          { areas, minTilePx: Math.min(...tiles.map((tile) => Math.min(tile.width, tile.height))) },
        );
      }
    ...
    await page.route(`${base}/review-fullscreen-host`, (route) =>
      route.fulfill({
        contentType: 'text/html',
        headers: { 'Permissions-Policy': 'fullscreen=()' },
        body: `<iframe src="${base}/games/${game}/" allow="fullscreen 'none'" style="width:390px;height:844px"></iframe>`,
      }),
    );
    ...
    record(
      `${game}: actual iframe Permissions Policy denial has Chinese feedback`,
      await frame.locator('#game-display-notice').innerText(),
    );
    await context.close();
  }
}
try {
  if (process.argv.includes('--failures')) {
    await failures();
  } else if (process.argv.includes('--layouts')) {
    await layouts();
    ...
    for (const game of games.length ? games : ['cops-robbers', 'letters-words2']) {
      ...
      if (!process.argv.includes('--probe')) {
        ...
        await snap(a, 'results');
        await snap(b, 'results');
        record(
          `${game}: both clients agree on final persisted results`,
          a.room.results.map((entry) => ({
            playerId: entry.playerId,
            result: entry.result,
            rank: entry.after.me?.rank,
          })),
        );
        ...
        assert.equal(a.room.status, 'waiting');
        assert.equal(b.room.status, 'waiting');
        record(`${game}: both players enter the same rematch`, {
          previousCode,
          nextCode: a.room.code,
        });
      }
      await a.context.close();
      await b.context.close();
