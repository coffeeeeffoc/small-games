...
const runtime = Number(process.env.COMPETITION_PORT || 43002),
  kart = Number(process.env.KART_SERVER_PORT || 43003),
  port = Number(process.env.COMPETITION_GATEWAY_PORT || 43010);
const mime = {
...
const server = createServer(async (req, res) => {
  ...
  if (url.pathname.startsWith('/api/competition/v1/')) {
    if (
      url.pathname.startsWith('/api/competition/v1/internal/') ||
      !/^\/(?:api\/competition\/v1\/(?:sessions\/(?:guest|platform)|me|boards\/[^/]+|rooms(?:\/[^/]+)?(?:\/(?:ready|leave|rematch|actions|role|initiative))?))$/.test(
        url.pathname,
      )
    ) {
      res.writeHead(404);
    ...
    const proxy = httpRequest(
      {
        ...
        path: req.url,
        method: req.method,
        headers: { ...req.headers, host: `127.0.0.1:${runtime}` },
      },
      (upstream) => {
  ...
  if (url.pathname === '/health') {
    const probe = httpRequest(
      { host: '127.0.0.1', port: runtime, path: '/health', timeout: 3000 },
      (upstream) => {
  ...
  try {
    ...
    if (decoded.split('/').some((part) => part.startsWith('.')) || /[\\\0]/.test(decoded))
    ...
    if (!resolved.startsWith(rootReal + path.sep) || !(await stat(resolved)).isFile())
      throw new Error('Denied path');
...
server.on('upgrade', (req, socket, head) => {
  ...
  const upstream = connect(kart, '127.0.0.1', () => {
    upstream.write(
      `${req.method} /kart HTTP/${req.httpVersion}\r\n` +
...
server.listen(port, '127.0.0.1', () =>
  console.log(
    `Game test gateway http://127.0.0.1:${port} (static build + public API + /kart only)`,
  ),
);
