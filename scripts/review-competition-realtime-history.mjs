...
const out = new URL(
  `../outputs/independent-realtime-history${new URL(base).hostname === '127.0.0.1' ? '' : '-public'}/`,
  import.meta.url,
);
...
async function player(game, who) {
  ...
  await page.addInitScript(() => {
    ...
    proto.clearRect = function (...args) {
      if (this.canvas.matches('[data-play]')) globalThis.__reviewPaint = [];
    ...
    proto.fillText = function (text, x, y, ...args) {
      if (this.canvas.matches('[data-play]')) {
        ...
        const width = this.measureText(text).width,
          size = Number(this.font.match(/([\d.]+)px/)?.[1] || 14);
        const start =
          this.textAlign === 'center' ? x - width / 2 : this.textAlign === 'right' ? x - width : x;
        (globalThis.__reviewPaint ||= []).push({
          text: String(text),
          x: (start * transform.a + transform.e) / ratio,
          y: (y * transform.d + transform.f) / ratio,
          w: (width * transform.a) / ratio,
          h: (size * transform.d) / ratio,
        });
      }
  ...
  page.on('response', async (response) => {
    if (/assets\/competition\/.*webp/.test(response.url()))
      p.images.push({ url: new URL(response.url()).pathname, status: response.status() });
    if (!response.ok()) return;
    if (response.url().includes('/api/competition/v1/rooms')) {
      const body = await response.json().catch(() => null);
      if (body?.players && body.code) p.room = body;
    }
    if (response.url().includes('/api/competition/v1/boards/'))
      p.board = await response.json().catch(() => null);
  });
  ...
  const local = await readFile(
    new URL(`../apps/shell-web/dist/games/${game}/competition.js`, import.meta.url),
  );
  assert(served.equals(local), 'running competition bundle must equal current gateway build');
  if (who === 'a')
    record(`${game}: current served bundle`, {
...
async function snap(p, name) {
  ...
  await writeFile(
    new URL(`${p.game}-${p.who}-${name}.json`, out),
    JSON.stringify(
      { room: p.room, text: await p.page.locator('dialog[open]').innerText(), images: p.images },
      null,
      2,
    ),
  );
...
async function tap(p, x, y, action = false) {
  ...
  assert(
    ...
    `visible tap ${x},${y} in ${rect.width}x${rect.height}`,
  );
...
async function start(game) {
  ...
  if (process.env.REVIEW_EDGES) {
    ...
    record(`${game}: copied invitation received in second browser`, {
      pathname: new URL(invitation).pathname,
      code: a.room.code,
    });
  ...
  record(`${game}: real identities create/join/ready`, {
    room: a.room.code,
    players: a.room.players.map((p) => p.id),
...
async function finish(players) {
  for (const p of players)
    await expect.poll(() => p.room?.status, { timeout: 150000 }).toBe('finished');
  ...
  for (const p of players) {
    ...
    record(`${p.game}/${p.who}: common server result and leaderboard`, {
      results: p.room.results,
      board: p.board,
  ...
  for (const p of players) {
    ...
    const directRematch = p.page
      .locator('[data-details]')
      .getByRole('button', { name: /^(再来一局|再次挑战)$/ });
  ...
  record(`${players[0].game}: both clients rematch into same new room`, {
    previous,
    next: players[0].room.code,
  });
  await players[0].page.getByRole('button', { name: '退出 PK', exact: true }).click();
}
async function history(players) {
  for (const p of players) {
    if (process.env.REVIEW_PLAY_LANDSCAPE && p.who === 'a')
    ...
    for (let round = 1; round <= 5; round++) {
      ...
      if (round === 1 && p.who === 'a') {
        ...
        await tap(p, box.width * 0.75, 54, true);
        assert(p.room.state.hint, 'hint received from server');
        await snap(p, 'hint');
      }
      const box = await p.page.locator('[data-play]').boundingBox();
      const wide = box.width > 600 && box.height < 460;
      await tap(p, box.width * 0.25, 54); // Map tab, then a real Beijing-area map tap.
      const rect = {
        x: 10,
        y: 80,
        w: box.width - 20,
        h: Math.max(80, box.height - (wide ? 138 : 205)),
      };
      await tap(
        p,
        rect.x + rect.w / 2 + ((116.4 - 20) * rect.w) / 360,
        rect.y + rect.h / 2 - ((39.9 - 15) * rect.h) / 170,
      );
      await tap(p, box.width / (wide ? 4 : 2), box.height - (wide ? 26 : 74));
      const columns = rect.h < 255 ? 6 : 3,
        rows = 12 / columns;
      const keyW = (box.width - 20 - (columns - 1) * 6) / columns;
      const keyH = Math.max(28, Math.min(48, (rect.h - 55) / rows - 5));
      for (const key of ['1', '9', '0', '0']) {
        const index = ['1', '2', '3', '4', '5', '6', '7', '8', '9', '清空', '0', '退格'].indexOf(
          key,
        );
        await tap(
          p,
          10 + (index % columns) * (keyW + 6) + keyW / 2,
          130 + Math.floor(index / columns) * (keyH + 5) + keyH / 2,
        );
      }
      if (round === 1) await snap(p, 'year-input');
      await tap(p, box.width * (wide ? 0.75 : 0.5), box.height - 26, true);
      ...
      record(`history/${p.who}: round ${round} server answer`, {
        score: p.room.state.answer.score,
        penalty: p.room.state.answer.penalty,
      });
...
function streetGeometry(state, width, height) {
  const nodes = state.map.nodes,
    minX = Math.min(...nodes.map((n) => n.x)) - 55,
    minY = Math.min(...nodes.map((n) => n.y)) - 65;
  const mapW = Math.max(...nodes.map((n) => n.x)) + 55 - minX,
    mapH = Math.max(...nodes.map((n) => n.y)) + 55 - minY;
  const bottom = Math.max(150, height - 106),
    scale = Math.max(0.05, Math.min((width - 16) / mapW, (bottom - 40) / mapH));
  return {
    bottom,
    scale,
    x: (width - mapW * scale) / 2 - minX * scale,
    y: 40 + (bottom - 40 - mapH * scale) / 2 - minY * scale,
  };
}
async function streetMove(p, cop, point) {
  ...
  const count = p.room.state.cops.length,
    bw = Math.min(84, (box.width - 24 - 6 * count) / (count + 1));
...
async function street(players) {
  ...
  for (const p of players)
    record(`street/${p.who}: actual canvas commands`, {
      phase: p.room.state.phase,
      caught: p.room.state.caught,
      elapsedMs: p.room.state.elapsedMs,
      actions: p.actions.length,
    });
}
async function edges(players) {
  ...
  for (const action of [
    { type: 'submit', score: 999999, elapsedMs: 1 },
    a.game === 'cops-robbers-realtime'
      ? { type: 'move', cop: 99, x: 0, y: 0 }
      : { type: 'guess', point: { lat: 91, lng: 0 }, year: 0 },
  ]) {
    const error = await a.page.evaluate(
      async ({ code, seq, action }) => {
        try {
          await globalThis.__competition.request(`/rooms/${code}/actions`, {
            body: JSON.stringify({ seq: seq + 1, action }),
          });
          return null;
        } catch (error) {
          return error.code;
  ...
  record(`${a.game}: forged score and illegal operation rejected`, {
    unchangedSeq: a.room.seq === beforeSeq,
  });
  ...
  const before = a.room.state.elapsedMs,
    beforeServerTime = a.room.serverNow;
  ...
  await expect
    .poll(() => a.page.locator('dialog [data-status]').innerText(), { timeout: 15000 })
    .not.toMatch(/网络|不可用/);
  assert(a.room.serverNow > beforeServerTime + 1500);
  if (a.game === 'cops-robbers-realtime') assert(a.room.state.elapsedMs > before + 1500);
  record(`${a.game}: offline recovery`, {
    offlineText,
    before,
    after: a.room.state.elapsedMs,
    roomUnchanged: code === a.room.code,
  });
...
async function layoutCheck(p, label) {
  ...
  if (p.game === 'cops-robbers-realtime') {
    for (const text of paint.filter((t) => /^(留守|[123] 号|比赛无法暂停)/.test(t.text))) {
      if (text.y + text.h / 2 > box.height || text.y - text.h / 2 < 0)
        issues.push(`clipped control: ${text.text}`);
    }
  } else {
    ...
    if (
      clue &&
      feedback &&
      clue.x < feedback.x + feedback.w &&
      feedback.x < clue.x + clue.w &&
      Math.abs(clue.y - feedback.y) < (clue.h + feedback.h) / 2
    )
      issues.push('clue overlaps map feedback');
  }
...
async function outage(game) {
  ...
  const code = p.room.code,
    id = p.room.players[p.room.you].id;
  record('OUTAGE_READY: waiting for coordinator to suspend the real tunnel', { code, id });
  await expect
    .poll(() => p.page.locator('[data-status]').innerText(), { timeout: 90000 })
    .toMatch(/网络不可用|服务.*不可用/);
  await snap(p, 'tunnel-unavailable');
  record('OUTAGE_OBSERVED', { text: await p.page.locator('[data-status]').innerText(), code });
  await expect
    .poll(() => p.page.locator('[data-status]').innerText(), { timeout: 90000 })
    .toMatch(/等待双方准备/);
  assert.equal(p.room.code, code);
  assert.equal(p.room.players[p.room.you].id, id);
...
try {
  const games = process.env.REVIEW_GAME
    ? [process.env.REVIEW_GAME]
    : process.env.REVIEW_OUTAGE
      ? ['vibeJam-myself-history-guess']
      : ['vibeJam-myself-history-guess', 'cops-robbers-realtime'];
  for (const game of games) {
    if (process.env.REVIEW_OUTAGE) {
      await outage(game);
      continue;
    }
    ...
    if (process.env.REVIEW_EDGES) await edges(players);
      ...
      if (game === 'vibeJam-myself-history-guess') await history(players);
      else await street(players);
      await finish(players);
    }
  ...
  await writeFile(
    new URL(`${name}-${process.env.REVIEW_GAME || 'all'}-report.json`, out),
