...
const psql=async(input,database='small_games')=>runCommand('docker',[...compose,'exec','-T','postgres','psql','-U','platform_owner','-d',database,'-v','ON_ERROR_STOP=1'],{cwd:root,input});
if(mode==='stop'){await runCommand('docker',[...compose,'stop','postgres'],{cwd:root});}
else if(mode==='up'||mode==='test-db') {
  await runCommand('docker',[...compose,'up','-d','--wait','postgres'],{cwd:root});
  if(mode==='test-db') {
    await psql("SELECT 'CREATE DATABASE competition_test OWNER platform_owner' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname='competition_test')\\gexec\n");
  ...
  let key;try{key=(await readFile(keyFile,'utf8')).trim();}catch(error){if(error.code!=='ENOENT')throw error;key=randomBytes(32).toString('hex');await writeFile(keyFile,key);}
  const port=process.env.COMPETITION_PORT||'43002',gateway=process.env.COMPETITION_GATEWAY_PORT||'43010';
  const origins=[`http://127.0.0.1:${gateway}`,`http://localhost:${gateway}`,...(process.env.COMPETITION_ORIGINS||'').split(',').filter(Boolean)].join(',');
  const env={...process.env,COMPETITION_ENABLED:'true',COMPETITION_INTERNAL_KEY:key,COMPETITION_ORIGINS:origins,
    RUNTIME_DATABASE_URL:process.env.RUNTIME_DATABASE_URL||'postgres://runtime_app:local-runtime-only@127.0.0.1:15432/small_games',
    PORT:port,COMPETITION_API_URL:`http://127.0.0.1:${port}/api/competition/v1`,KART_SERVER_PORT:process.env.KART_SERVER_PORT||'43003',
    KART_ALLOWED_ORIGINS:origins,KART_OUTBOX_DIR:fileURLToPath(new URL('../.scratch/competition/kart-outbox',import.meta.url))};
