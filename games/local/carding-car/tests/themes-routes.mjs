...
const loaded = async page => {
  await page.waitForFunction(() => globalThis.__kart && (!__kart.snapshot().loading || __kart.snapshot().loadError), {}, { timeout: 60000 });
...
async function select(page, field, id) {
  ...
  await loaded(page);
  const after = await snapshot(page), other = field === 'theme' ? 'route' : 'theme';
...
try {
  ...
  observe(page);
  await page.goto(url); assert.equal(await page.title(), '浪湾卡丁车'); await loaded(page);
  for (const theme of themes) {
    await select(page, 'theme', theme.id);
    for (const route of process.env.KART_MATRIX_SMOKE ? routes.filter(r => r.id === theme.id) : routes) {
      ...
      assert.equal(await page.evaluate(async () => {
        ...
        return cc.director.getScene().getChildByName('KartGame').children.filter(n => n.name === 'SelectedTheme' && n.active).length;
      }), 1);
      ...
      if (theme.id === route.id || route.id === 'highland' || route.id === 'seaside') {
        await page.keyboard.press('Enter'); await page.keyboard.down('ArrowUp');
        await page.waitForFunction(() => __kart.snapshot().time > 1.2);
        await page.keyboard.up('ArrowUp');
        assert.ok((await snapshot(page)).player.speed > 0);
        await page.screenshot({ path: fileURLToPath(new URL(`${theme.id}-${route.id}.png`, reports)) });
        await page.keyboard.press('KeyP'); await page.keyboard.press('KeyG'); await loaded(page);
      }
    }
  ...
  for (let i = 0; i < 6; i++) { await page.keyboard.press('Digit1'); await page.keyboard.press('Digit2'); }
  const finalSelection = (await snapshot(page)).selection; await loaded(page);
  assert.deepEqual((await snapshot(page)).selection, finalSelection);
  await page.close();
  for (const viewport of [{ width: 844, height: 390 }, { width: 360, height: 800 }]) {
    const mobile = await browser.newPage({ viewport, hasTouch: true, isMobile: true, userAgent: 'Mozilla/5.0 (Linux; Android 14) AppleWebKit/537.36 Chrome/130.0.0.0 Mobile Safari/537.36' });
    observe(mobile); await mobile.goto(url); await loaded(mobile);
    ...
    for (const [i, field] of ['theme', 'route', 'vehicle', 'driver'].entries()) {
      ...
      await tap(710, 226 + i * 40); await loaded(mobile);
      const after = (await snapshot(mobile)).selection;
      assert.notEqual(after[field], before[field]);
      for (const other of Object.keys(before).filter(k => k !== field)) assert.equal(after[other], before[other]);
    }
    await mobile.screenshot({ path: fileURLToPath(new URL(`menu-${viewport.width}.png`, reports)) });
    await tap(480, 395); await mobile.waitForFunction(() => __kart.snapshot().time > 1);
  ...
  await writeFile(new URL('validation.json', reports), JSON.stringify({ combinations: evidence, errors }, null, 2));
  console.log(`PASS: ${evidence.length} combinations, independent touch choices, persistence, rapid switching; no errors.`);
