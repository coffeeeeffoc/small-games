...
test('old world saves migrate to the same theme and route without losing car or driver', () => {
  for (const route of routes) {
    const saved = readSelection(
      JSON.stringify({ world: route.id, vehicle: 'rally', driver: 'ranger' }),
    );
    ...
    assert.equal(
      routeRecordKey(saved.route),
      route.id === 'seaside' ? 'coastline-records-v1' : `kart-records-v1-${route.id}`,
    );
  }
  assert.deepEqual(readSelection('{"theme":"glacier","route":"bad"}'), {
    ...defaultSelection,
    theme: 'glacier',
  });
  assert.deepEqual(readSelection('{"theme":"bad","route":"highland"}'), {
    ...defaultSelection,
    route: 'highland',
  });
});
  ...
  for (const route of routes) {
    ...
    for (const theme of themes) {
      ...
      for (const shape of scenery.shapes ?? []) {
        assert.ok(
          [shape.x, shape.y, shape.z, shape.sx, shape.sy, shape.sz, shape.yaw ?? 0].every(
            Number.isFinite,
          ),
          `${theme.id}/${route.id}`,
        );
      ...
      for (const model of scenery.models ?? [])
        assert.ok([model.x, model.y, model.z, model.scale].every(Number.isFinite));
      for (const { geometry: mesh } of scenery.meshes ?? []) {
        assert.ok(
          [...mesh.positions, ...mesh.normals, ...(mesh.uvs ?? [])].every(Number.isFinite),
          `${theme.id}/${route.id}: finite mesh`,
        );
...
test('landmark clearance includes both branches and the full footprint, with vertical space for supports', () => {
  ...
  const main = pointAt(track, 50),
    branch = track.shortcut[20];
