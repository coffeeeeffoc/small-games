...
export function journeyFrame(progress, index, reduced = false) {
  ...
  return {
    opacity: local < 0 ? entering : local < 1 ? 1 : 0,
    scale: reduced ? 1 : 1.025 + clamp(local) * .15 + (1 - entering) * .12,
    x: reduced ? 0 : (index % 2 ? -1 : 1) * clamp(local) * 1.8,
    y: reduced ? 0 : (1 - entering) * 9 - clamp(local) * 3,
    blur: reduced ? 0 : leaving * 1.8,
...
export function startJourney({ initialId, getState, onCollect, onLeave, onSound = () => {} }) {
    ...
    <div class="journey-scroll" tabindex="0" aria-label="大理风景画卷，上下滑动或使用方向键游览">
      <div class="journey-track"><div class="journey-stage">
        ${STORIES.map((story, i) => `<article class="journey-scene" data-place="${story.id}" aria-label="${PLACES[i].name}">
          <img class="journey-image" src="./assets/journey/${story.id}.webp" alt="${PLACES[i].name}的精细微缩景观插画" draggable="false" decoding="async">
  ...
  function draw() {
    ...
    active = Math.min(STORIES.length - 1, Math.floor(progress + .21));
    ...
    scenes.forEach((scene, index) => {
      ...
      style.setProperty("--scene-opacity", frame.opacity);
      style.setProperty("--scene-scale", frame.scale);
      style.setProperty("--scene-x", `${frame.x}vw`);
      style.setProperty("--scene-y", `${frame.y}vh`);
      style.setProperty("--scene-blur", `${frame.blur}px`);
  ...
  function collect() {
    ...
    try {
      ...
      ctx.drawImage(image, (canvas.width - image.naturalWidth * ratio) / 2, (canvas.height - image.naturalHeight * ratio) / 2, image.naturalWidth * ratio, image.naturalHeight * ratio);
