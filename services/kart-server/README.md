...
`share` 启动游戏服务并允许该网页来源；它不登录或启动隧道客户端。打开输出的公网 `/play/` 地址创建房间，点击“邀请好友”复制包含房间码的链接。若浏览器不允许复制，手动分享当前公网游戏地址和房间码。免费域名变化后，用新地址重新运行 `share` 并分享新链接。
...
环境变量：`PORT`（43003）、`HOST`（127.0.0.1）、`KART_MAX_ROOMS`（16，最大 100）、`KART_ALLOWED_ORIGINS`（逗号分隔的完整网页 origin，无路径）。独立 H5 页面必须把自己的 origin 加入允许列表；未配置时仅允许本机网页，原生无 Origin 连接可用。
