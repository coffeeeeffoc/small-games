export function playerName(player, peers = []) {
  ...
  while (
    ...
    duplicates.some(
      (p) =>
        (p.playerId || p.id) !== id &&
        (p.playerId || p.id || '').slice(0, length) === id.slice(0, length),
    )
  )
    length++;
...
export function scoreText(game, score, secondary = 0) {
  ...
  if (game === 'letters-words2')
    return `${Math.floor(score / 1000000)}词 · 正确率${((score % 1000000) / 100).toFixed(2)}% · ${time}`;
...
export function gapText(game, board) {
  ...
  if (game === 'letters-words2') {
    ...
    return words
      ? `距上一名多完成 ${words} 词`
      : `距上一名正确率提高 ${(score / 100).toFixed(2)} 个百分点`;
  }
  return `距上一名 ${score} 分`;
}
