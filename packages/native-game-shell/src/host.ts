...
import { createNativeAdProvider } from './ads.js';
...
export function createNativeGameHost(
  sdk: NativeSdk,
  ...
  return createBrowserGameHost({
    ...
    content,
    storagePrefix: `${options.platformId}:${manifest.gameId}:`,
    storage: {
      getItem(key) {
    ...
    advertising: () =>
      createAdRuntime({
        ...
        host: createNativeAdProvider(sdk, options.adUnitId),
        telemetry,
      }),
  });
}
