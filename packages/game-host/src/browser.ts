...
import { createBrowserStoragePort } from './storage.js';
...
export function createBrowserGameHost(
  options: GameHostOptions & { storage: KeyValueStorage; storagePrefix?: string },
): GameHost {
  ...
  return Object.freeze({
    ...
    ...createDefaultPorts(
      options,
      createBrowserStoragePort(options.storage, prefix, options.online ?? true),
      session,
    ),
  });
}
