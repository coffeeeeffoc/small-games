...
按 2026-09-10 的迁移要求，tower-defense-game、xiangqi-five、office-slacking 保留各自仓库，在 `games/submodules/*` 作为 Git submodule 纳入 pnpm workspace。这是 ADR-0001 对独立仓库 Game 的例外；现有平台包仍使用普通 workspace 目录。
...
## 2026-09-14：按 Git 归属整理目录

原 `apps/game-*` 使用 `git mv` 移至 `games/local/game-*`，由父仓库直接管理源码；四个现有子模块使用 `git mv` 移至 `games/submodules/*`，保留原提交指针和子仓库历史。两类目录均加入 pnpm workspace。目录分类仅表示 Git 归属，不决定 Game Host 或静态 iframe 的加载方式。包名、Game ID、现有存档命名空间和大厅 `/games/<id>/` 地址保持不变。
