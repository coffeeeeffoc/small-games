...
后续用户明确授权全部提交推送到 `main`，已将根仓库与象棋子模块成果推送 main。根仓库 `.github/workflows/pages.yml` 会随 main 更新发布 Pages；这不代表业务后端已部署生产。最新入口、昵称与退出改进见 [PK 界面与昵称记录](2026-09-23-pk-ui-profiles.md)，未通过的平台与评分门槛保持原记录。
